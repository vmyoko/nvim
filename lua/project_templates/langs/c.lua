...
return function(on_complete)
  creator.create({
    ...
    preview = {
      ...
      "", " Standards:", "   C99, C11, C17, C23", "", " Generates:",
      "  - main.c", "  - CMakeLists.txt", "  - Makefile", "", " 󰄉 Ready to code.",
    },
    fields = {
    ...
    local main = core.read_template("c", "main.c")
    local cmake = core.read_template("c", data.type == "1" and "exe.txt" or "lib.txt")
    cmake = string.format(cmake, data.name, data.ver, data.name)

    core.write_file(full_path .. "/main.c", main)

...
local function get_oldfiles()
  ...
  if #results == 0 then
    table.insert(results, { label = "(No recent files)", action = function() end })
  end
  return results
end
...
local function get_projects()
  ...
  if ok and core.get_projects then
    ...
    for i, p in ipairs(projects) do
      ...
      table.insert(results, {
        ...
        action = function()
          vim.cmd("cd " .. vim.fn.fnameescape(p.path))
  ...
  if #results == 0 then
    table.insert(results, { label = "(No projects found)", action = function() end })
  end
  return results
end
...
local function draw()
  ...
  local title_row = string.rep(" ", left_pad)
    .. make_top_border(1)
    .. gap
    .. make_top_border(2)
    .. gap
  ...
  for i = 1, max_items do
    ...
    for col_idx = 1, 3 do
      ...
      if item then
        ...
        if col_idx == 2 then
          offset = offset + string.len(pad_string(c1, col1_w)) + 3
        end
        if col_idx == 3 then
          offset = offset + string.len(pad_string(c1, col1_w)) + 3 + string.len(pad_string(c2, col2_w)) + 3
        end
  ...
  local footer_row = string.rep(" ", left_pad)
    .. make_bottom_border(col1_w)
    .. gap
    .. make_bottom_border(col2_w)
    .. gap
    .. make_bottom_border(col3_w)
...
-- ==============================================================================
  -- APPLY HIGHLIGHTS USING EXTMARKS (MODERN NEOVIM ALTERNATIVE)
  -- ==============================================================================
  vim.api.nvim_buf_clear_namespace(buf, ns, 0, -1)

  local start_logo_line = math.max(1, math.floor(win_height / 2) - 15)
  local total_logo_lines = #logo_lines

  -- 1. Logo Highlights
  for i = 0, total_logo_lines - 1 do
    local row = start_logo_line + i
    vim.api.nvim_buf_set_extmark(buf, ns, row, 0, {
      end_col = string.len(content[row + 1] or ""),
      hl_group = "Function",
    })
  end

  -- 2. Greeting, Date, and Time Highlights
  local text_start_line = start_logo_line + total_logo_lines
  
  vim.api.nvim_buf_set_extmark(buf, ns, text_start_line + 1, 0, {
    end_col = string.len(content[text_start_line + 2] or ""),
    hl_group = "Special",
  })
  vim.api.nvim_buf_set_extmark(buf, ns, text_start_line + 2, 0, {
    end_col = string.len(content[text_start_line + 3] or ""),
    hl_group = "Function",
  })
  vim.api.nvim_buf_set_extmark(buf, ns, text_start_line + 3, 0, {
    end_col = string.len(content[text_start_line + 4] or ""),
    hl_group = "Comment",
  })

  -- 3. Header Borders Highlight
  local header_line_idx = text_start_line + 6 -- FIXED: Changed from +5 to +6
  vim.api.nvim_buf_set_extmark(buf, ns, header_line_idx, 0, {
    end_col = string.len(content[header_line_idx + 1] or ""),
    hl_group = "FloatBorder",
  })

  -- 4. Active Selection Menu Item Highlights
  for _, h in ipairs(hl_data) do
    local offset = left_pad
    if h.col == 2 then
      offset = offset + string.len(pad_string(h.c1, col1_w)) + 3
    end
    if h.col == 3 then
      offset = offset + string.len(pad_string(h.c1, col1_w)) + 3 + string.len(pad_string(h.c2, col2_w)) + 3
    end

    -- Destaca o ícone indicador prefixo '  '
    vim.api.nvim_buf_set_extmark(buf, ns, h.line - 1, offset, {
      end_col = offset + 5, -- 1 space (1 byte) +  (3 bytes) + 1 space (1 byte) = 5 bytes
      hl_group = "Function"
    })
    
    -- Destaca o texto do item selecionado
    vim.api.nvim_buf_set_extmark(buf, ns, h.line - 1, offset + 5, {
      end_col = offset + string.len(h.text),
      hl_group = "Function"
    })
  end

  -- 5. Footer Borders Highlight
  local footer_line_idx = header_line_idx + max_items + 1
  vim.api.nvim_buf_set_extmark(buf, ns, footer_line_idx, 0, {
    end_col = string.len(content[footer_line_idx + 1] or ""),
    hl_group = "FloatBorder",
  })
...
M.open = function()
  ...
  map("<Down>", function()
    ...
    if #columns[col].items > 0 then
      ...
      if state.sel[col] > #columns[col].items then
        state.sel[col] = 1
      end
    end
    draw()
  ...
  map("<Up>", function()
    ...
    if #columns[col].items > 0 then
      state.sel[col] = state.sel[col] - 1
      if state.sel[col] < 1 then
        state.sel[col] = #columns[col].items
      end
    end
    draw()
  end)
